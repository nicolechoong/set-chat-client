...
async function onCreateChat (chatID, chatName, validMemberPubKeys, invalidMembers) {
    ...
    for (const mem of validMemberPubKeys.keys()) {
        ...
        keyMap.set(mem, naclUtil.encodeUTF8(validMemberPubKeys.get(mem)));
    }
    ...
    store.getItem(chatID).then(async (chatInfo) => {
        return new Promise(async (resolve) => {
            for (const mem of validMemberPubKeys.keys()) {
                ...
                broadcastToMembers({
                    id: nacl.hash(naclUtil.encodeUTF8(`${localUsername}:${sentTime}`)),
                    type: "add",
                    op: op,
                    from: localUsername,
                    name: mem,
...
function getDeps (operations) {
    ...
    for (const op of operations) {
        const hashedOp = nacl.hash(naclUtil.encodeUTF8(concatOp(op)));
...
async function generateOp (action, chatID, pk2 = null, ops = new Set()) {
    ...
    return new Promise(function(resolve) {
        ...
        console.log(`encoded ${naclUtil.encodeUTF8(concatOp(op)) instanceof Uint8Array}, length of sig ${nacl.sign.detached(new TextEncoder().encode(concatOp(op)), keyPair.secretKey).length}`);
        op["sig"] = nacl.sign.detached(naclUtil.encodeUTF8(concatOp(op)), keyPair.secretKey);
            resolve(op);
    });
}
...
async function sendOperations (chatID, username) {
    ...
    store.getItem(chatID).then((chatInfo) => {
        const stringedOps = [...chatInfo.metadata.operations].map(op => {
            console.log(op.sig);
            op.sig = naclUtil.decodeUTF8(op.sig);
            console.log(op.sig);
            return op; 
        });
        sendToMember({
...
async function receivedOperations (ops, chatID, username) {
    ...
    store.getItem(chatID).then((chatInfo) => {
        ops = ops.map(op => { console.log(op.sig); op.sig = naclUtil.encodeUTF8(op.sig); return op; });
...
function getOpFromHash(ops, hashedOp) {
    ...
    for (const op of ops) {
        if (hashedOp == nacl.hash(naclUtil.encodeUTF8(concatOp(op)))) {
...
function verifyOperations (ops) {
    ...
    console.log(`${typeof createOp.sig}     ${naclUtil.encodeUTF8(createOp.pk) instanceof Uint8Array}`)
    ...
    const hashedOps = new Set(ops.map((op) => nacl.hash(naclUtil.encodeUTF8(JSON.stringify(op)))));

    for (const op of otherOps) {
        // valid signature
        console.log(`${op.sig instanceof Uint8Array}     ${naclUtil.encodeUTF8(op.pk1) instanceof Uint8Array}`)
        console.log(`sig length ${createOp.sig.length}`);
        if (!nacl.sign.detached.verify(naclUtil.encodeUTF8(concatOp(op)), op.sig, op.pk1)) { console.log("op verification failed: key verif failed"); return false; }
...
loginBtn.addEventListener("click", function (event) { 
    ...
    if (loginInput.length > 0 && isAlphanumeric(loginInput)) {
        sendToServer({ 
            type: "login", 
            name: loginInput,
            pubKey: naclUtil.decodeUTF8(keyPair.publicKey)
        });
    }
});
